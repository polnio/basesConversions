...
export function binToHex(bin) {
    ...
    for (let i = bin.length; i > 0; i -= 4) {
        ...
        if (!binKeysNumber.includes(query))
            return '';
        r = Object.entries(convertObj).find(([h, b]) => parseInt(b) === query)[0] + r;
    }
    return r;
}
export function hexToDec(nb) {
    let r = 0;
    nb.split('').forEach(n => {
...
export function hexToBin(hex) {
    let r = '';
    hex.split('').forEach(c => {
        if (!Object.keys(convertObj).includes(c.toUpperCase()))
            return '';
        r += convertObj[c.toUpperCase()];
    });
    return trim(r, '0');
}
